...
export const PROVIDERS: ProviderInfo[] = [
  ...
  },
  {
    id: "finviz",
    name: "Finviz Elite",
    purpose: "Screens the whole market (not just this app's built-in ~500-name list) for setups like 'well below its 52-week high' — how new opportunities get discovered, not just symbols already on your watchlist. Optional; discovery works without it, just over a narrower list of stocks.",
    required: false,
    fields: [{ field: "auth_token", label: "Auth token", placeholder: "Elite export auth token" }],
    url: "https://finviz.com/elite",
    steps: [
      "This needs a paid Finviz Elite subscription (finviz.com/elite) — the free tier does not include the export feature this uses.",
      "Once subscribed, log in and open your account/profile page on finviz.com.",
      "Find your Elite \"auth\" token (shown on your account page, or visible in the URL any time you use the CSV export button on the screener).",
  ...
  },
  {
    id: "fintel",
    name: "Fintel",
    purpose: "Short interest and institutional-positioning data — what large holders and short sellers are doing on a stock. Optional enrichment; some fields may be unavailable depending on your Fintel plan.",
    required: false,
    fields: [{ field: "api_key", label: "API key", placeholder: "Fintel API key" }],
    url: "https://fintel.io/api-developers",
    steps: [
      "This needs a paid Fintel plan with API access — check fintel.io/api-developers for current tiers.",
      "Log in to fintel.io, then open the Developer Dashboard at fintel.io/u/dev.",
      "Click \"Generate Key\" and copy the key shown.",
      "Paste it here. Use \"Test connection\" — some fields depend on your specific plan, so a partial result is normal.",
    ],
  },
  {
    id: "edgar",
